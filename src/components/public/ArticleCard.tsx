...
export default function ArticleCard({ article }: { article: Article }) {
  ...
  return (
    <Link href={`/artikel/${article.slug}`} className="block group reveal">
      <article className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl hover:border-green-200 transition-all duration-300 h-full flex flex-col">
        ...
        <div className="p-6 flex-1 flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-4 mb-3 text-xs font-semibold text-gray-500">
              {article.kategori && (
                <span className="flex items-center gap-1 bg-green-50 text-green-700 px-2.5 py-1 rounded-lg">
                  <Tag size={12} />
                  {article.kategori}
                </span>
              )}
